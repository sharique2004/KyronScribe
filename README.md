...
- **Voice dictation** — a mic button on the transcript records audio (MediaRecorder) and transcribes it **server-side through Gemini's multimodal input** (`POST /api/transcribe`), then appends the text to the transcript. Deliberately not the browser SpeechRecognition API (disabled in Brave and others); works in every browser the app runs in. Mock mode returns canned dictation keylessly.
- **Self-improving diagnostic memory** — a reflection + retrieval loop, no model retraining: when a saved encounter *revises* the patient's prior primary diagnosis (different ICD-10 root), the AI reflects on both encounters and stores a **clinical lesson** (missed signals, discriminating workup, forward-looking summary) in `clinical_lessons`, embedded with the same local MiniLM engine as the ICD search. Every future generation semantically matches the transcript against stored lessons and injects strong matches into the system prompt — the UI shows "Applying learned diagnostic pattern" plus a "Learned patterns applied" block in the context panel, and the patient's chart gains a **Diagnostic revisions** card. Behavior provably improves as the practice accumulates revisions.
